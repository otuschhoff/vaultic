...
**Status: implemented (2026-09-14); portable protocol, adapter, command, lifecycle, security, and fuzz coverage complete. Privileged native-client mount runs on macOS, Linux, and FreeBSD remain explicit opt-in release validation before this phase is marked complete.**
...
Phase 30 owns the persistent-cache backend, representation, budget, trust, resize, corruption-fallback, and broker-lifetime matrix. Phase 31 verifies that both FUSE and NFS route regular-file reads through the same protocol-neutral `snapshotfs` file and repository `ReadLogicalFileRange` entry point, without adding a protocol-specific persistent cache. Phase 30's local-disk, S3, librados, restart, resize, trust, and Ceph pressure suites remain authoritative for that shared manager.
...
Performance baselines should compare sequential and random reads with FUSE over the same repository and Phase 30 cache policy, separating server cache effects from kernel/NFS client cache hits. Those measurements are release characterization, not a functional completion gate; bounded memory, transfer sizes, connections, requests, and slow-client isolation remain automated correctness gates.

## Completion evidence

The implementation extracts the immutable repository view into `internal/snapshotfs`, keeps FUSE as an adapter over that view, and adds an isolated `internal/nfs` adapter around the pinned `go-nfs` RPC/XDR implementation. `vaultic serve nfs` resolves one snapshot or subfolder, binds separate fixed NFS and mount-protocol TCP listeners, defaults to loopback, enforces explicit non-loopback acknowledgement and CIDRs, prints native-client commands, writes an optional protected readiness record, and shuts down with the command and repository lifecycle.

Portable automated tests cover snapshot lookup, attributes, deterministic directories, symlinks, bounded and cross-blob reads, FUSE/NFS contract equivalence, authenticated and restart-stale handles, directory verifiers, read-only mutation responses, mount protocol operations, separate program listeners, malformed and oversized requests, no UDP listener, source filtering, limits, idle shutdown, readiness ownership, and command validation. Fuzz seed corpora exercise strict path normalization and authenticated file-handle parsing/tampering during ordinary `go test` runs.

The native-client integration test is deliberately gated by `VAULTIC_NFS_INTEGRATION_TESTS`, a supported OS, root privileges, and native mount/unmount tool availability. It starts the real server on isolated loopback ports, mounts with fixed NFS and mount ports, browses, reads, copies and hashes fixture data, verifies a mutation fails, unmounts, and always stops the server. Privileged macOS, Linux, and FreeBSD executions have not been run as part of this completion work and remain external release validation; no native mount result is inferred from portable RPC tests.

## Exit criterion

The implementation gate is met when `vaultic serve nfs SNAPSHOT` exposes exactly the selected immutable snapshot or subfolder through read-only NFSv3 TCP without requiring FUSE or root on the Vaultic host; raw-wire tests prove every mutation returns `NFS3ERR_ROFS`, handles and cookies cannot escape the export, malformed requests remain bounded, projected modes apply without trusting client-claimed root, and shutdown remains bounded while retaining eventual cleanup ownership. FUSE and NFS must share the tested `snapshotfs` and repository range-read path, with Phase 30 remaining authoritative for persistent-cache trust and budget behavior. Non-loopback deployment must require explicit acknowledgement and network restrictions.

The release-validation gate additionally runs the opt-in native mount harness on macOS, Linux, and FreeBSD to mount, browse, copy, hash, reject mutation, and unmount the export. Those privileged runs are pending and are not inferred from portable protocol tests.
