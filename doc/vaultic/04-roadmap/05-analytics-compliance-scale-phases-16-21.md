...
Policies support one or many alternatives such as normal `2-of-4` Azure custodians and break-glass `2-of-3` offline custodians. Every membership, threshold, member-key rotation, or policy change requires an already unlocked daemon, generates a fresh root wrapping secret and fresh shares, rewraps both capsule payloads, and publishes and mirrors a new immutable generation without changing either underlying secret. Status reports the minimum effective threshold across every current access path; a retained 1-of-1 bootstrap member, password keyfile, direct master-key copy, single-key escrow, overlapping Entra authorization, or other complete-key bypass makes the repository non-compliant and must never be presented as four-eyes protected. Seat separation is verifiable only for Azure members, whose contributions carry validated Entra object IDs; offline seats have no verifiable holder identity, so one person holding two offline shares defeats an offline quorum undetectably. Effective-threshold output therefore labels offline groups as custody-assumed rather than verified, and their separation is enforced by the documented custody runbook. A quorum-compliant repository consequently has no unattended startup path at all: every daemon restart — crash, patching, power loss — blocks in the locked state until a quorum completes, cached or persisted warm-restart unlock material would be a bypass and is prohibited, and the documentation must state these on-call expectations explicitly.

Because a locked daemon cannot call custodian-scoped Azure keys itself, unlock uses an interactive contribution session. Capsule ciphertext is deliberately non-secret and widely replicated, so the primary residual attack is a fake locked daemon phishing custodians for shares; sessions are therefore authenticated, not merely encrypted. The daemon reads the capsule without opening SlateDB and creates a one-time session using an authenticated, reviewed HPKE construction with a fresh ephemeral key, nonce, transcript, and expiry, signed by the daemon identity key pinned in the capsule policy. `vaultic index unlock contribute` verifies that signature against the capsule, displays a short session fingerprint for out-of-band confirmation between custodians, and embeds in the signed transcript the capsule generation the custodian's tooling last observed. Each custodian authenticates directly with Entra or opens an offline share locally, unwraps only their member share, encrypts it to the verified session, submits it, and zeroizes local plaintext. The daemon validates the Entra tenant and immutable principal object ID where applicable and rejects unsigned or wrongly signed sessions, duplicate share indexes, duplicate contributing principals, foreign transcripts, expired or replayed contributions, and contributions for another repository or generation. After $K$ valid shares it reconstructs the root secret in locked memory, authenticates and unwraps both the metadata DEK and repository master key, zeroizes all intermediate material, and permanently closes the session. Because the identity private key necessarily rests in plaintext on the daemon host, this raises share harvesting from "anyone holding public capsule ciphertext" to "an attacker controlling the daemon host who still deceives $K$ custodians"; the fingerprint ceremony and custodian generation checks address that remainder.
