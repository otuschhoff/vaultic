...
4. Extend the envelope with `any_of`, `all_of`, and threshold expressions containing a single offline bootstrap member, hardware-backed member alternatives, or threshold groups. Support one or many `offline-argon2id`, `offline-keyfile`, `yubikey-piv`, `fido2-hmac-secret`, and `azure-key-vault` members in any configured grouping, including a hardware seat where multiple enrolled YubiKeys unwrap the same member share as `1-of-N` alternatives; preserve Phase 18 envelopes only as an explicit migration input, not as a parallel quorum-compliant access path.
