...
### A third wrapped payload: the sealed topology

`RecoveryCapsule` (format 2) carries `metadata_dek` and `repository_master_key` as `WrappedPayload`s under the quorum-recovered root key. Format 3 adds one payload:
...
RecoveryCapsule {
  ...
  sealed_topology:         WrappedPayload,   // new, format >= 3
}
```

`sealed_topology` wraps a canonical-JSON **topology document** with its own purpose-separated key (`HKDF(root, repository_id, "sealed-topology-v1")`) and AEAD associated data binding capsule `logical_id`, `generation`, and `repository_id`, so a payload cannot be transplanted between capsules or generations. A capsule without the payload (format 2) remains valid and is reported as `topology: external`.
...
| Phase 22 bootstrap-topology manifest | unchanged, credential-free | retained as the metadata-outage discovery path for hosts that have a master-key lease but an old-format capsule; when a format-3 capsule is present the manifest is verified to match and is otherwise ignored |
...
- `topology-read` — returns the document with the `credentials` map removed. Replaces the Phase 22 `topology-discovery` HKDF key for format-3 capsules. Granted to `vaultic` and `vaulticdb` alongside their existing leases.
...
| Credential | OAuth 2.0 refresh token for an installed-app client, scope `drive.file` (only files the app created; Vaultic never needs broader access to the user's Drive), plus `client_id`/`client_secret`. `oauth2.TokenSource` refreshes access tokens in memory; nothing is written to disk. Service accounts with domain-wide delegation are also supported (`gcp-service-account-json` kind with a `subject`) for Workspace tenants. |
...
1. Bump `CAPSULE_FORMAT` to 3 with an optional `sealed_topology: WrappedPayload`; extend `recover_from_shares` and `RecoveredKeys` to unwrap it under `sealed-topology-v1`; keep format 2 readable and report `topology: external`.
...
5. Teach `vaultic` repository opening to prefer capsule topology (`--topology-source capsule`), build backends from structured endpoints plus leased credentials, verify agreement with the encrypted `config`, and fall back to Phase 22 manifest discovery only for format-2 capsules.
