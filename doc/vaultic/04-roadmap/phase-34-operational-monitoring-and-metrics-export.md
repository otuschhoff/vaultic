...
[← Phase 33](phase-33-index-check-scalability-and-performance.md) · [Phase 35 →](phase-35-native-smb-backup-source.md)
...
|---|---|---|---|
| M0 | Complete | The [M0 contract and durability review](phase-34-m0-contract.md) freezes metric/profile/artifact schemas, shared fixtures, the complete owner-to-metric/action matrix, backend acknowledgement assumptions, crash review, and the pinned-engine rejection proof. | Ordered-prefix token implementation and persistent import resume are explicitly deferred to M2g; per-commit durability remains the normal default. |
| M1 | Complete | The [M1 bounded-accounting evidence](phase-34-m1-evidence.md) covers Go wait, action, dependency, and sampled-correlation primitives; lock-free bounded Rust active timing; import/check production adapters; schema, concurrency, overflow, cancellation, result-equivalence, and enabled/disabled overhead checks. | Broad production boundary coverage remains M3; injection and scenario profiles remain M2. |
| M2 | Complete | The [M2 isolated-scenario evidence](phase-34-m2-evidence.md) covers target-gated Go/Rust injection, stream/cache/scratch boundaries, post-success response delivery and recovery, six real action matrices, ordered durability tokens, and barrier-driven restart/crash tests. | Small fixtures are not representative throughput evidence. Broad production boundary wiring remains M3 and representative infrastructure comparisons remain M7. |
| M3 | Complete | The [M3 production-boundary evidence](phase-34-m3-evidence.md) maps inherited VaulticDB/queue/cache telemetry and owner-scoped source, repository transfer, restore destination, placement, maintenance, cache lifecycle, broker/human wait, and VFS/FUSE/NFS accounting. Deterministic attribution, bounded-cardinality, outcome, capability-preservation, and enabled/disabled overhead checks pass. | Representative infrastructure throughput and cache-effectiveness measurements remain M7; unavailable and non-applicable observations are not reported as zero. |
| M4 | Complete | The [M4 authenticated-snapshot evidence](phase-34-m4-evidence.md) covers protected VaulticDB and key-broker status handoff, fixed transport/cardinality bounds, process/capture identity, compatibility, authorization, redaction, stale/unavailable handling, collector isolation and aggregate golden JSON. | Terminal interaction remains M5; exporter behavior remains M6. |
| M5 | Complete | The [M5 aggregate-monitoring evidence](phase-34-m5-evidence.md) covers bounded 1/5/15-minute rates and percentiles, keyboard views and sorting, pause/reset, narrow/resize behavior, raw-terminal restoration, asynchronous reconciliation, cancellation, honest unavailable states, and operator output. | Export delivery remains M6. |
| M6 | Complete | The [M6 optional-export evidence](phase-34-m6-evidence.md) covers the vendor-neutral asynchronous worker, opt-in InfluxDB v2 delivery, protected token sources, bounded batches/queue/retry/shutdown, reset markers, export health, maximum-cardinality CPU/memory/response measurements, overflow/staleness, and a blocked-endpoint production data-path isolation run. Rust services contain no exporter clients. | Representative cross-workload and infrastructure evidence remains M7. |
| M7 | Blocked | Phase 32 P3/P7 artifacts provide import-only synthetic and representative results with explicit revision and coverage limits; M2 supplies complete bounded correctness matrices for all six actions. | Phase 33 checker evidence, all shared-profile current-revision representative runs, and representative infrastructure evidence do not exist. They must not be inferred from the M2 correctness fixtures. |
