...
- **Attribute-Based & Sampled Storage Verification:** `vaultic index verify-storage` / `vaultic verify-packs` provides query filters (`--tier`, `--backend`, `--created-after/before`, `--min-size`, `--max-size`, `--retention-status`, `--pack-type`) and statistical sampling controls (`--all`, `--sample-count`, `--sample-percent`).
- **Multi-Level Check Verification:** Supports Level 1 (header decryption), Level 2 (full encrypted-object SHA-256), and Level 3 (full payload unpack) checks, with automated warm-up invocation for cold Glacier packs.
...
All major cryptographic, logging, and verification gaps have been addressed in [**Phase 17**](../04-roadmap/phase-17-iso27001-gdpr-compliance-azure-key-vault-syslog-and-storage-verification.md):
