...
func newIndexPathIndexCommand(globalOptions *global.Options) *cobra.Command {
	...
	command := &cobra.Command{
		...
		Long:              "Build or refresh pv path-index entries for explicitly selected paths. The index is optional and derived; path history falls back to the immutable walk when it is absent." + indexExitStatus,
		Args:              cobra.NoArgs,
...
func runIndexPathIndex(ctx context.Context, options indexPathIndexOptions, globalOptions global.Options, term ui.Terminal) (any, error) {
	...
	if err == nil && !globalOptions.JSON {
		printer.P("scanned %d snapshots; changed %d path bindings; overflow paths %d; estimated bytes %d\n", result.SnapshotsScanned, result.BindingsChanged, result.OverflowPaths, result.BytesWritten)
	}
	return result, err
}
