...
func newIndexStatsCommand(globalOptions *global.Options) *cobra.Command {
	...
	command := &cobra.Command{
		...
		Long: "Report pack counts and sizes for the repository, grouped by tier, type, or lifecycle state. " +
			"Totals are read from the constant-time aggregate records unless a filter, grouping, or verification requires the catalog." +
			indexExitStatus,
		Args:              cobra.NoArgs,
...
func runIndexPacks(ctx context.Context, options indexPacksOptions, globalOptions global.Options, term ui.Terminal) (maintenance.PacksResult, error) {
	...
	if !globalOptions.JSON {
		...
		if result.Undecidable != 0 {
			...
			printer.P(
				"undecidable %d (creation time %d; retention %d; usage %d): "+
					"these packs lack the fact the filter asks about and were excluded for lack of evidence\n",
				result.Undecidable,
...
func newIndexHistoryCommand(globalOptions *global.Options) *cobra.Command {
	...
	command := &cobra.Command{
		...
		Long: "Report a time series over the pack history rollups. Repack and promotion churn are reported separately from net growth, " +
			"because rewriting or moving data is not growth." + indexExitStatus,
		Args:              cobra.NoArgs,
		DisableAutoGenTag: true,
		RunE: func(command *cobra.Command, _ []string) error {
...
func newIndexHistoryPruneCommand(globalOptions *global.Options) *cobra.Command {
	...
	command := &cobra.Command{
		...
		Long: "Roll up raw pack history events into buckets, then truncate raw events and buckets past their retention. " +
			"Rolling up before truncating is what allows raw events to be discarded without losing the periods they describe." + indexExitStatus,
		Args:              cobra.NoArgs,
		DisableAutoGenTag: true,
		RunE: func(command *cobra.Command, _ []string) error {
...
func runIndexHistoryPrune(
	ctx context.Context,
	options indexHistoryPruneOptions,
	globalOptions global.Options,
	term ui.Terminal,
) (maintenance.HistoryRetentionResult, error) {
...
func newIndexBackendsCommand(globalOptions *global.Options) *cobra.Command {
	...
	command := &cobra.Command{
		...
		Long: "Report each configured backend and, with --compare, cross-check a backend listing against the pack catalog. " +
			"Listing is opt-in because on an archival backend it has a real cost." + indexExitStatus,
		Args:              cobra.NoArgs,
		DisableAutoGenTag: true,
		RunE: func(command *cobra.Command, _ []string) error {
...
func runIndexBackends(ctx context.Context, options indexBackendsOptions, globalOptions global.Options, term ui.Terminal) (BackendsResult, error) {
	...
	if hotCold {
		targets = []backendTarget{
			{id: "hot", role: "hot", ingest: true, readEnabled: true, lister: hot},
			{id: "cold", role: "cold", ingest: true, readEnabled: true, lister: cold},
		}
	}
