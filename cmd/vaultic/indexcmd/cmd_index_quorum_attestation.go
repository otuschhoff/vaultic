...
type quorumBypassOptions struct {
	Statements quorumBypassStatements
	ValidFor   time.Duration
}

func (options quorumBypassOptions) finalize() error {
	statements := options.Statements
	if !statements.NoPlaintextKeyExports || !statements.NoExternalStandaloneEscrow || !statements.GenerationAnchorsCurrent ||
		!statements.BrokerCredentialsProtected ||
		!statements.NoWarmRestartMaterial ||
		!statements.OfflineSharesSeparate {
		return fmt.Errorf("every bypass inventory confirmation is required")
	}
	if options.ValidFor <= 0 || options.ValidFor > 90*24*time.Hour {
...
func newIndexKeysQuorumGenerateAttestationKeyCommand(globalOptions *global.Options) *cobra.Command {
	...
	command := &cobra.Command{
		Use:               "generate-attestation-key",
		Short:             "Generate a bypass-attestation signing keypair",
		Args:              cobra.NoArgs,
...
func newIndexKeysQuorumAttestBypassesCommand(globalOptions *global.Options) *cobra.Command {
	...
	options := quorumBypassOptions{ValidFor: 30 * 24 * time.Hour}
	command := &cobra.Command{
		Use:               "attest-bypasses",
		Short:             "Sign an inventory of non-discoverable complete-key bypasses",
		Args:              cobra.NoArgs,
		DisableAutoGenTag: true,
		...
		RunE: func(_ *cobra.Command, _ []string) error {
			...
			attestation := quorumBypassAttestation{
				...
				IssuedUnix: now.Unix(), ExpiresUnix: now.Add(options.ValidFor).Unix(), Statements: options.Statements,
			}
			...
			result := map[string]any{
				...
				"repository_id":      capsule.RepositoryID(),
				"capsule_generation": capsule.Generation(),
				"expires_unix":       attestation.ExpiresUnix,
			}
			if globalOptions.JSON {
				globalOptions.Term.Print(ui.ToJSONString(result))
			} else {
	...
	command.Flags().
		DurationVar(&options.ValidFor, "valid-for", 30*24*time.Hour, "attestation validity duration (maximum 2160h)")
	command.Flags().
		BoolVar(&options.Statements.NoPlaintextKeyExports, ("confirm-no-plaintext-key-exports"), false, ("attest that no persistent plaintext complete-key " +
			"export exists"))
	command.Flags().
		BoolVar(
			&options.Statements.NoExternalStandaloneEscrow,
			"confirm-no-external-standalone-escrow",
			false,
			"attest that no standalone escrow copy remains outside managed inventory",
		)
	command.Flags().
		BoolVar(
			&options.Statements.GenerationAnchorsCurrent,
			"confirm-generation-anchors-current",
			false,
			"attest that every custodian generation anchor is current",
		)
	command.Flags().
		BoolVar(
			&options.Statements.BrokerCredentialsProtected,
			"confirm-broker-credentials-protected",
			false,
			"attest that broker credentials cannot bypass quorum",
		)
	command.Flags().
		BoolVar(
			&options.Statements.NoWarmRestartMaterial,
			"confirm-no-warm-restart-material",
			false,
			"attest that no complete-key warm-restart material persists",
		)
	command.Flags().
		BoolVar(
			&options.Statements.OfflineSharesSeparate,
...
func quorumAttestationFindings(capsule interface {
	...
	if attestation.IssuedUnix > now.Unix() || attestation.ExpiresUnix <= now.Unix() ||
		attestation.ExpiresUnix-attestation.IssuedUnix > int64(90*24*time.Hour/time.Second) {
