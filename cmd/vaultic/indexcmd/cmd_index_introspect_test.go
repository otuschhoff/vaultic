...
func TestBackendsNoListPerformsZeroBackendRequests(t *testing.T) {
	hot := &countingBackend{location: "hot:/x", files: map[backend.FileType][]backend.FileInfo{
		...
		backend.KeyFile:       nil,
		backend.LockFile:      nil,
		backend.SnapshotFile:  nil,
		backend.IndexFile:     nil,
		backend.ConfigFile:    nil,
		backend.SlateDBFile:   nil,
		backend.BootstrapFile: nil,
		backend.StagingFile:   nil,
	}}
