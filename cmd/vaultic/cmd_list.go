...
func runList(ctx context.Context, gopts global.Options, args []string, term ui.Terminal, listAllowedArgsUseString string) error {
	...
	return repo.List(ctx, t, func(id vaultic.ID, _ int64) error {
		printer.S("%s", id)
		return nil
	})
}
...
func packfileList(ctx context.Context, repo vaultic.Repository, snapshotID string, printer vaultic.Printer) error {
	...
	bar.SetMax(uint64(1))
	err = data.FindUsedBlobs(ctx, repo, []vaultic.ID{*sn.Tree}, usedBlobs, bar)
	bar.Done()
	if err != nil {
		return err
	}

	snapPacks := vaultic.NewIDSet()
