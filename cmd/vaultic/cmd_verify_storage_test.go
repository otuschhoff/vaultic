...
func TestParseVerificationOptionsCurrentStatusAndSizes(t *testing.T) {
	...
	if _, _, err := parseVerificationOptions(verifyStorageOptions{CurrentStatus: true, All: true, Level: "header", Concurrency: 1}); err == nil || !strings.Contains(err.Error(), "sampling mode") {
		t.Fatalf("current status accepted sampling mode: %v", err)
	}
	if _, _, err := parseVerificationOptions(verifyStorageOptions{All: true, Level: "header", MinSize: 201, MaxSize: 200, Concurrency: 1}); err == nil || !strings.Contains(err.Error(), "min-size") {
		t.Fatalf("invalid size range accepted: %v", err)
	}
	future := time.Now().Add(time.Hour).UTC().Format(time.RFC3339)
	if _, _, err := parseVerificationOptions(verifyStorageOptions{All: true, Level: "header", NotVerifiedSince: future, Concurrency: 1}); err == nil || !strings.Contains(err.Error(), "future") {
