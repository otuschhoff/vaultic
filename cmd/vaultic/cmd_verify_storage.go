...
func newVerifyPacksCommand(globalOptions *global.Options) *cobra.Command {
	return newVerifyStorageCommand(globalOptions, "verify-packs", "Verify selected physical pack placements", cmdGroupAdvanced)
}
...
func newVerifyStorageCommand(globalOptions *global.Options, use, short, group string) *cobra.Command {
	...
	command := &cobra.Command{Use: use, Short: short, GroupID: group, Args: cobra.NoArgs, DisableAutoGenTag: true, RunE: func(command *cobra.Command, _ []string) error {
...
func runVerifyStorage(ctx context.Context, options verifyStorageOptions, globalOptions global.Options) error {
	...
	if options.ErrorHistory && (options.All || options.SampleCount != 0 || options.SamplePercent != 0 || options.ErrorsOnly || len(options.Tiers) != 0 || len(options.Backends) != 0 || len(options.StorageClasses) != 0 || len(options.PackTypes) != 0 || options.CreatedAfter != "" || options.CreatedBefore != "" || options.MinSize != 0 || options.MaxSize != 0 || options.RetentionStatus != "" || options.NotVerifiedSince != "") {
	...
	_ = observability.Emit(ctx, observability.Event{Severity: severity, Category: observability.CategoryIntegrity, Component: "verify-storage", Message: "storage verification completed", Fields: map[string]any{"candidates": result.Candidates, "verified": result.Verified, "integrity_errors": result.IntegrityErrors, "operational_errors": result.OperationalErrors}})
	return runErr
}

func parseVerificationOptions(options verifyStorageOptions) (indexverification.Options, schema.VerificationLevel, error) {
	selection := indexverification.Options{Tiers: map[schema.PackTier]bool{}, Backends: map[uint64]bool{}, StorageClasses: map[string]bool{}, PackTypes: map[schema.PackType]bool{}, RetentionStatus: options.RetentionStatus, SampleCount: options.SampleCount, SamplePercent: options.SamplePercent, Seed: options.Seed, ErrorsOnly: options.ErrorsOnly}
	levels := map[string]schema.VerificationLevel{"header": schema.VerificationHeader, "checksum": schema.VerificationChecksum, "full": schema.VerificationFull, "unpack": schema.VerificationFull}
	...
	if options.ErrorHistory && options.CurrentStatus {
		return selection, 0, fmt.Errorf("--error-history and --current-status are mutually exclusive")
	}
	if (!options.CurrentStatus && modes != 1) || (options.CurrentStatus && modes != 0) {
		if options.CurrentStatus {
			return selection, 0, fmt.Errorf("--current-status cannot be combined with a sampling mode")
		}
		return selection, 0, fmt.Errorf("exactly one of --all, --sample-count, or --sample-percent is required")
	}
	for _, value := range options.Tiers {
		tier, ok := map[string]schema.PackTier{"hot": schema.TierHot, "cold": schema.TierCold, "mirrored": schema.TierMirrored, "single": schema.TierSingle, "unknown": schema.TierUnknown}[value]
		if !ok {
			return selection, 0, fmt.Errorf("invalid --tier %q", value)
		}
		selection.Tiers[tier] = true
	}
	for _, value := range options.PackTypes {
		packType, ok := map[string]schema.PackType{"data": schema.PackData, "tree": schema.PackTree, "mixed": schema.PackMixed, "unknown": schema.PackUnknown}[value]
		if !ok {
			return selection, 0, fmt.Errorf("invalid --pack-type %q", value)
		}
	...
	for _, value := range options.Backends {
		...
		if err != nil || backend == 0 {
			return selection, 0, fmt.Errorf("invalid --backend %q", value)
		}
	...
	parseTime := func(name, value string) (*int64, error) {
		if value == "" {
			return nil, nil
		}
	...
	var err error
	if selection.CreatedAfter, err = parseTime("created-after", options.CreatedAfter); err != nil {
		return selection, 0, err
	}
	if selection.CreatedBefore, err = parseTime("created-before", options.CreatedBefore); err != nil {
		return selection, 0, err
	}
	if selection.NotVerifiedSince, err = parseTime("not-verified-since", options.NotVerifiedSince); err != nil {
		return selection, 0, err
	}
	if selection.CreatedAfter != nil && selection.CreatedBefore != nil && *selection.CreatedAfter >= *selection.CreatedBefore {
		return selection, 0, fmt.Errorf("--created-after must be earlier than --created-before")
	}
	if selection.NotVerifiedSince != nil && *selection.NotVerifiedSince > time.Now().Unix() {
		return selection, 0, fmt.Errorf("--not-verified-since cannot be in the future")
	}
	...
	if options.MinSize > 0 && options.MaxSize > 0 && options.MinSize > options.MaxSize {
		return selection, 0, fmt.Errorf("--min-size must not exceed --max-size")
	}
	if options.RetentionStatus != "" && options.RetentionStatus != "active" && options.RetentionStatus != "expired" {
		return selection, 0, fmt.Errorf("invalid --retention-status %q", options.RetentionStatus)
	}
	if options.Concurrency <= 0 || options.SampleCount < 0 || options.SamplePercent < 0 || options.SamplePercent > 100 {
		return selection, 0, fmt.Errorf("invalid sampling or concurrency value")
	}
...
func verificationStatus(candidates []indexverification.Candidate) []verificationStatusEntry {
	...
	for index, candidate := range candidates {
		result[index] = verificationStatusEntry{PackID: hex.EncodeToString(candidate.PackID[:]), Backend: candidate.Backend, Tier: candidate.Pack.Tier.String(), StorageClass: candidate.Placement.StorageClass, Result: verificationResultName(candidate.State.Result), Level: verificationLevelName(candidate.State.LastAttemptLevel), Class: verificationClassificationName(candidate.State.Classification), State: candidate.State, HasState: candidate.HasState}
	}
	return result
}
...
func verificationHistory(ctx context.Context, store *daemon.SchemaStore, limit int) ([]verificationHistoryEntry, error) {
	...
	for {
		...
		for _, item := range items {
			...
			result = append(result, verificationHistoryEntry{Time: key.EventTime, Sequence: key.Revision, PackID: hex.EncodeToString(key.ID[:]), Backend: key.Backend, Type: verificationEventTypeName(event.Type), Level: verificationLevelName(event.Level), Class: verificationClassificationName(event.Classification), Event: event})
...
func verificationLevelName(value schema.VerificationLevel) string {
	return map[schema.VerificationLevel]string{schema.VerificationHeader: "header", schema.VerificationChecksum: "checksum", schema.VerificationFull: "full"}[value]
}

func verificationResultName(value schema.VerificationResult) string {
	return map[schema.VerificationResult]string{schema.VerificationUnknown: "unknown", schema.VerificationHealthy: "healthy", schema.VerificationOperationalError: "operational-error", schema.VerificationIntegrityError: "integrity-error"}[value]
}

func verificationClassificationName(value schema.VerificationClassification) string {
	return map[schema.VerificationClassification]string{schema.VerificationNoError: "none", schema.VerificationMissing: "missing", schema.VerificationSizeMismatch: "size-mismatch", schema.VerificationChecksumMismatch: "checksum-mismatch", schema.VerificationHeaderAuthentication: "header-authentication", schema.VerificationPayloadDecrypt: "payload-decrypt", schema.VerificationDecompression: "decompression", schema.VerificationWarmupTimeout: "warm-up-timeout", schema.VerificationTransport: "transport", schema.VerificationCancelled: "cancelled"}[value]
}

func verificationEventTypeName(value schema.VerificationEventType) string {
	return map[schema.VerificationEventType]string{schema.VerificationDetected: "detected", schema.VerificationChanged: "changed", schema.VerificationResolved: "resolved"}[value]
}
