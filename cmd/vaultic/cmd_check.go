...
func prepareCheckCache(opts CheckOptions, gopts *global.Options, printer vaultic.Printer) (cleanup func()) {
	...
	tempdir, err := os.MkdirTemp(cachedir, "vaultic-check-cache-")
	if err != nil {
...
func selectPacksByBucket(allPacks map[vaultic.ID]int64, bucket, totalBuckets uint) map[vaultic.ID]int64 {
...
func selectRandomPacksByPercentage(allPacks map[vaultic.ID]int64, percentage float64) map[vaultic.ID]int64 {
...
func selectRandomPacksByFileSize(allPacks map[vaultic.ID]int64, subsetSize int64, repoSize int64) map[vaultic.ID]int64 {
...
type checkSummary struct {
	...
	BrokenPacks     []string `json:"broken_packs"`         // run "vaultic repair packs ID..." and "vaultic repair snapshots --forget" to remove damaged files
	HintRepairIndex bool     `json:"suggest_repair_index"` // run "vaultic repair index"
	HintPrune       bool     `json:"suggest_prune"`        // run "vaultic prune"
}
...
func (*jsonErrorPrinter) NewCounter(_ string) vaultic.Counter {
	return vaultic.NoopCounter
}

func (*jsonErrorPrinter) NewCounterTerminalOnly(_ string) vaultic.Counter {
	return vaultic.NoopCounter
}
