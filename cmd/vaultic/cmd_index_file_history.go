...
func newIndexFileHistoryCommand(globalOptions *global.Options) *cobra.Command {
	...
	command := &cobra.Command{
		...
		Long:              "Resolve path history through immutable directory revisions in commit order, or scan one inode's revision prefix. Legacy repositories fail explicitly rather than answering from current state." + indexExitStatus,
...
func newIndexPathAtCommand(globalOptions *global.Options) *cobra.Command {
	...
	command := &cobra.Command{
		...
		Long:              "Resolve a path by walking immutable directory revisions from a snapshot root and report the directory revision chain used." + indexExitStatus,
		Args:              cobra.ExactArgs(1),
		DisableAutoGenTag: true,
		RunE: func(command *cobra.Command, args []string) error {
...
func runIndexPathAt(ctx context.Context, options indexPathAtOptions, target string, globalOptions global.Options, term ui.Terminal) (indexhistory.PathAtResult, error) {
	...
	if err == nil && !globalOptions.JSON {
		printer.P("%s at %s: present=%v covered=%v inode=%d revision=%d type=%s\n", result.Path, result.SnapshotID, result.Present, result.Covered, result.Inode, result.Revision, result.NodeType)
...
func openHistoryStore(ctx context.Context, daemonOptions indexDaemonOptions, globalOptions global.Options, term ui.Terminal) (indexhistory.Store, vaultic.Printer, func(), error) {
