...
snapshot tree as shown by "vaultic ls".
...
func addBlobs(bs vaultic.AssociatedBlobSet, node *data.Node) {
	if node == nil {
		return
	}
...
type DiffStatsContainer struct {
	...
	BlobsBefore, BlobsAfter, BlobsCommon vaultic.AssociatedBlobSet `json:"-"`
}

// updateBlobs updates the blob counters in the stats struct.
func updateBlobs(repo vaultic.Loader, blobs vaultic.AssociatedBlobSet, stats *DiffStat, printError func(string, ...any)) {
...
func (c *Comparer) printDir(ctx context.Context, mode string, stats *DiffStat, blobs vaultic.AssociatedBlobSet, prefix string, id vaultic.ID) error {
...
func (c *Comparer) collectDir(ctx context.Context, blobs vaultic.AssociatedBlobSet, id vaultic.ID) error {
...
func runDiff(ctx context.Context, opts DiffOptions, gopts global.Options, args []string, term ui.Terminal) error {
	...
	be, err := vaultic.MemorizeList(ctx, repo, vaultic.SnapshotFile)
	if err != nil {
		return err
	}
	...
	stats.BlobsBefore.Insert(vaultic.BlobHandle{Type: vaultic.TreeBlob, ID: *sn1.Tree})
	stats.BlobsAfter.Insert(vaultic.BlobHandle{Type: vaultic.TreeBlob, ID: *sn2.Tree})
