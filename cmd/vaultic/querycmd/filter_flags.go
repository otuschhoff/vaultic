...
func initExtendedSnapshotFilter(flags *pflag.FlagSet, filt *data.SnapshotFilter, withLast bool) {
	...
	flags.Var(
		&stringListFlag{target: &filt.FilterPaths},
		"filter-paths",
		"only consider snapshots containing this `path[,path,...]` list (can be specified multiple times)",
	)
	flags.Var(
		&stringListFlag{target: &filt.PathsExact},
		"filter-paths-exact",
		"only consider snapshots whose paths exactly match this `path[,path,...]` list (can be specified multiple times)",
	)
	flags.Var(
		&tagListFlag{target: &filt.FilterTags},
		"filter-tags",
		"only consider snapshots containing this `tag[,tag,...]` list (can be specified multiple times)",
	)
	flags.Var(
		&stringListFlag{target: &filt.TagsExact},
		"filter-tags-exact",
		"only consider snapshots whose tags exactly match this `tag[,tag,...]` list (can be specified multiple times)",
	)
	...
	flags.Var(
		&sizeRangeFlag{min: &filt.SizeMin, max: &filt.SizeMax},
		"filter-size",
		"only consider snapshots with a total size within `min..max` (rustic size units accepted)",
	)
	flags.Var(
		&sizeRangeFlag{min: &filt.SizeAddedMin, max: &filt.SizeAddedMax},
		"filter-size-added",
		"only consider snapshots adding within `min..max` to the repository",
	)
	if withLast {
