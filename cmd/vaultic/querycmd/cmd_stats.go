...
import (
	...
	statsui "github.com/otuschhoff/vaultic/internal/ui/stats"
...
func runStats(ctx context.Context, opts StatsOptions, gopts global.Options, args []string, term ui.Terminal) error {
	...
	stats := &statsContainer{
		...
		fileBlobs:      make(map[string]vaultic.IDSet),
...
func countRawDataBlobs(repo *repository.Repository, stats *statsContainer, progress *statsui.Progress) error {
	for blobHandle := range stats.blobs.Keys() {
		packedBlobs := repo.LookupBlob(blobHandle)
		if len(packedBlobs) == 0 {
			return fmt.Errorf("blob %v not found", blobHandle)
		}
		blob := packedBlobs[0]
		stats.TotalSize += uint64(blob.CiphertextLength())
		if repo.Config().Version >= 2 {
			stats.TotalUncompressedSize += uint64(blob.UncompressedCiphertextLength())
			if blob.IsCompressed() {
				stats.TotalCompressedBlobsSize += uint64(blob.CiphertextLength())
				stats.TotalCompressedBlobsUncompressedSize += uint64(blob.UncompressedCiphertextLength())
			}
		}
		stats.TotalBlobCount++
		progress.Update(0, 1, uint64(blob.CiphertextLength()))
	}
...
func printStats(
	...
	printer.S("Stats in %s mode:", mode)
...
func countUniqueStatsNode(
	...
	fileID := makeFileIDByContents(node)
	if _, ok := stats.uniqueFiles[fileID]; ok {
		return nil
	}
	...
	if _, ok := stats.fileBlobs[nodePath]; !ok {
		stats.fileBlobs[nodePath] = vaultic.NewIDSet()
		stats.TotalFileCount++
	}
...
func statsDebug(ctx context.Context, repo vaultic.Repository, printer vaultic.Printer) error {
	printer.E("Collecting size statistics\n\n")
	for _, t := range []vaultic.FileType{vaultic.KeyFile, vaultic.LockFile, vaultic.IndexFile, vaultic.SnapshotFile, vaultic.PackFile} {
...
func statsDebugFileType(ctx context.Context, repo vaultic.Lister, tpe vaultic.FileType) (*sizeHistogram, error) {
	hist := newSizeHistogram(2 * repository.MaxPackSize)
	err := repo.List(ctx, tpe, func(_ vaultic.ID, size int64) error {
...
func statsDebugBlobs(ctx context.Context, repo vaultic.Repository) ([vaultic.NumBlobTypes]*sizeHistogram, error) {
	var hist [vaultic.NumBlobTypes]*sizeHistogram
