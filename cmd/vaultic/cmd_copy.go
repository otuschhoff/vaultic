...
func runCopy(ctx context.Context, opts CopyOptions, gopts global.Options, args []string, term ui.Terminal) error {
	...
	srcSnapshotLister, err := vaultic.MemorizeList(ctx, srcRepo, vaultic.SnapshotFile)
	if err != nil {
		return err
	}

	dstSnapshotLister, err := vaultic.MemorizeList(ctx, dstRepo, vaultic.SnapshotFile)
	if err != nil {
		return err
	}
...
func copyTreeBatched(ctx context.Context, srcRepo *repository.Repository, dstRepo vaultic.AppendRepository,
	...
	for {
		...
		err := dstRepo.WithBlobUploader(ctx, func(ctx context.Context, uploader vaultic.BlobSaverWithAsync) error {
...
func copyTree(ctx context.Context, srcRepo *repository.Repository, dstRepo vaultic.AppendRepository,
	...
	packList := vaultic.NewIDSet()
	var lock sync.Mutex

	enqueue := func(h vaultic.BlobHandle) {
		lock.Lock()
		defer lock.Unlock()
	...
	err := data.StreamTrees(ctx, srcRepo, vaultic.IDs{rootTreeID}, vaultic.NoopCounter, func(treeID vaultic.ID) bool {
		handle := vaultic.BlobHandle{ID: treeID, Type: vaultic.TreeBlob}
		...
		enqueue(vaultic.BlobHandle{ID: treeID, Type: vaultic.TreeBlob})
		...
		for item := range nodes {
			...
			for _, blobID := range item.Node.Content {
				enqueue(vaultic.BlobHandle{Type: vaultic.DataBlob, ID: blobID})
			}
		}
		return nil
	})
...
func copyStats(srcRepo vaultic.Repository, copyBlobs vaultic.AssociatedBlobSet, packList vaultic.IDSet, printer vaultic.Printer) uint64 {
	// count and size
	countBlobs := 0
	sizeBlobs := uint64(0)
