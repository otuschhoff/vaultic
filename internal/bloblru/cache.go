...
const overhead = len(vaultic.ID{}) + 64

// A Cache is a fixed-size LRU cache of blob contents.
// It is safe for concurrent access.
type Cache struct {
	mu sync.Mutex
	c  *simplelru.LRU[vaultic.ID, []byte]
...
func (c *Cache) add(id vaultic.ID, blob []byte) (old []byte) {
...
func (c *Cache) get(id vaultic.ID) ([]byte, bool) {
	c.mu.Lock()
	blob, ok := c.c.Get(id)
	c.mu.Unlock()
...
func (c *Cache) GetOrCompute(id vaultic.ID, compute func() ([]byte, error)) ([]byte, error) {
