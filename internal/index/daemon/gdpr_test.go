...
func TestExecuteGDPRForgetRedactsReferencesQueuesExclusivePackAndReplays(t *testing.T) {
	...
	target := schema.InodeRevision{
		Size:            20,
		UID:             targetUID,
		Known:           schema.KnownSize | schema.KnownUID | schema.KnownPath,
		ContentMode:     schema.ContentInline,
		ContentCount:    2,
		ContentIDs:      []schema.ID{exclusiveBlob, sharedBlob},
		SourcePath:      "/home/alice/data",
		FileContentHash: testSchemaID(9),
		HashKnown:       true,
		Freshness:       schema.FreshnessVerified,
	}
	external := schema.InodeRevision{
		Size:         10,
		UID:          601,
		Known:        schema.KnownSize | schema.KnownUID | schema.KnownPath,
		ContentMode:  schema.ContentInline,
		ContentCount: 1,
		ContentIDs:   []schema.ID{sharedBlob},
	...
	directory := schema.DirectoryRevision{
		Children:  []schema.DirectoryChild{{Name: "alice-secret", Inode: 10, Type: schema.NodeFile, MetadataKey: targetKey}},
		Freshness: schema.FreshnessVerified,
	}
	...
	blobs := []Mutation{
		{
			Key:   schema.BlobKey(exclusiveBlob),
			Value: encodeSchemaRecord(t, schema.BlobRecord{Locations: []schema.BlobLocation{{PackID: exclusivePack, Type: schema.BlobData}}}),
		},
		{
			Key:   schema.BlobKey(sharedBlob),
			Value: encodeSchemaRecord(t, schema.BlobRecord{Locations: []schema.BlobLocation{{PackID: sharedPack, Type: schema.BlobData}}}),
		},
		{
			Key: schema.PackKey(exclusivePack),
	...
	mutable := []Mutation{
		{
			Key:   schema.ReverseInodeKey(exclusiveBlob, 1, 10),
			Value: encodeSchemaRecord(t, schema.ReverseInodeRecord{LatestRevision: 1, State: schema.ReferenceCurrent}),
		},
		{
			Key:   schema.ReverseInodeKey(sharedBlob, 1, 10),
			Value: encodeSchemaRecord(t, schema.ReverseInodeRecord{LatestRevision: 1, State: schema.ReferenceCurrent}),
		},
		{
			Key:   schema.ReverseInodeKey(sharedBlob, 1, 11),
			Value: encodeSchemaRecord(t, schema.ReverseInodeRecord{LatestRevision: 1, State: schema.ReferenceCurrent}),
		},
		{
			Key:   schema.ReferenceCountKey(exclusiveBlob),
			Value: encodeSchemaRecord(t, schema.ReferenceCountRecord{TotalReferences: 1, DistinctInodes: 1, DistinctRevisions: 1}),
		},
		{
			Key:   schema.ReferenceCountKey(sharedBlob),
			Value: encodeSchemaRecord(t, schema.ReferenceCountRecord{TotalReferences: 2, DistinctInodes: 2, DistinctRevisions: 2}),
		},
		{
			Key: schema.PackPlacementKey(exclusivePack, 7),
			Value: encodeSchemaRecord(
				t,
				schema.PlacementRecord{State: schema.PlacementLive, Bytes: 100, RetentionSource: schema.RetentionBackend, MinRetentionUntil: 200},
			),
		},
		{
			Key:   schema.PackPlacementKey(sharedPack, 7),
			Value: encodeSchemaRecord(t, schema.PlacementRecord{State: schema.PlacementLive, Bytes: 100, RetentionSource: schema.RetentionUnknown}),
		},
		{
			Key:   schema.UserInodeKey(targetUID, 1, 10),
			Value: encodeSchemaRecord(t, schema.AnalyticsUserInodeRecord{LatestRevision: 1, PathSample: "/home/alice/data"}),
		},
		{
			Key:   schema.PathVersionKey(1, "home/alice/data", 1),
			Value: encodeSchemaRecord(t, schema.PathVersionRecord{State: schema.PathBound, NodeType: schema.NodeFile, Inode: 10, Revision: 1}),
		},
	}
	...
	if len(certificate.PurgedReferenceHashes) != 2 || len(certificate.PendingDeletion) != 1 || certificate.PendingDeletion[0].PackID != exclusivePack ||
		certificate.PendingDeletion[0].DeleteAfter != deletionDeadline {
