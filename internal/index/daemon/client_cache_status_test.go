...
func TestReadCacheStatusMappingPreservesAvailabilityAndTiers(t *testing.T) {
	...
	status := readCacheStatus(&vaulticdbv1.ReadCacheStatusResponse{
		...
		Metrics: &vaulticdbv1.ReadCacheMetrics{
			Hits: 7, Misses: 3, OriginReadsAvoided: 6,
	...
	if status.Metrics.AdmissionRejections != 15 || status.Metrics.AdmissionRejectionsReservation != 4 ||
		status.Metrics.AdmissionRejectionsBackgroundBudget != 5 || status.Metrics.AdmissionRejectionsBackgroundTask != 6 ||
		!status.Metrics.AdmissionRejectionReasonsAvailable {
		t.Fatalf("admission rejection metrics = %+v", status.Metrics)
	}
	...
	if unconfigured.Metrics.AdmissionRejectionReasonsAvailable {
		t.Fatal("legacy zero rejection reasons reported as available")
	}
}
