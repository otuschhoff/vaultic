...
func TestAzureKeyVaultUnwrapperReturnsProviderAcceptedPrincipal(t *testing.T) {
	...
	client := &http.Client{Transport: roundTripFunc(func(request *http.Request) (*http.Response, error) {
		if request.Method != http.MethodPost || request.URL.Path != "/keys/alice/version/unwrapkey" ||
			request.URL.Query().Get("api-version") != "7.4" {
		...
		return &http.Response{
			...
			Body: io.NopCloser(
				strings.NewReader(`{"value":"` + base64.RawURLEncoding.EncodeToString([]byte("wrapped-share")) + `"}`),
			),
	...
	if string(plaintext) != "wrapped-share" || principal.Authority != "entra" ||
		principal.TenantAccountOrProject != "tenant-a" ||
		principal.ImmutablePrincipalID != "object-a" {
...
func TestAzureKeyVaultUnwrapperFailsClosed(t *testing.T) {
	...
	expiredClaims := fmt.Sprintf(
		`{"aud":"https://vault.azure.net","tid":"tenant-a","oid":"object-a","exp":%d}`,
	...
	unwrapper, err := NewAzureKeyVaultUnwrapper(
		[]byte(azureTestToken("https://vault.azure.net", "tenant-a", "object-a")),
		client,
	)
	if err != nil {
		t.Fatal(err)
	...
	if _,
		...
		ExternalMemberContext{Provider: "azure-key-vault",
			KeyReference: ("https://example.vault.azure.net/keys/alice/version")},
