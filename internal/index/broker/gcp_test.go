...
func TestGoogleCloudKMSUnwrapperReturnsProviderAcceptedPrincipal(t *testing.T) {
	client := &http.Client{Transport: roundTripFunc(func(request *http.Request) (*http.Response, error) {
		...
		case "cloudkms.googleapis.com":
			if request.Method != http.MethodPost ||
				!strings.HasSuffix(
					request.URL.Path,
					"/projects/project-a/locations/global/keyRings/ring/cryptoKeys/alice:decrypt",
				) {
			...
			return &http.Response{
				...
				Body: io.NopCloser(
					strings.NewReader(
						`{"plaintext":"` + base64.StdEncoding.EncodeToString([]byte("wrapped-share")) + `"}`,
					),
				),
		...
		case "oauth2.googleapis.com":
			return &http.Response{
				StatusCode: http.StatusOK,
				Body: io.NopCloser(
					strings.NewReader(`{"sub":"subject-a","email":"alice@example.com","expires_in":"3600"}`),
				),
				Header: make(http.Header),
			}, nil
		default:
	...
	plaintext, principal, err := unwrapper.UnwrapMember(
		...
		ExternalMemberContext{
			...
			MemberID:       "alice",
			Provider:       "gcp-kms",
			KeyReference:   "projects/project-a/locations/global/keyRings/ring/cryptoKeys/alice",
			Purpose:        "purpose-a",
		},
		[]byte("ciphertext"),
	)
	...
	if string(plaintext) != "wrapped-share" || principal.Authority != "gcp-iam" ||
		principal.TenantAccountOrProject != "project-a" ||
		principal.ImmutablePrincipalID != "subject-a" {
...
func TestGoogleCloudKMSUnwrapperFailsClosed(t *testing.T) {
	...
	if _,
		...
		ExternalMemberContext{Provider: "gcp-kms",
			KeyReference: ("projects/project-a/locations/global/keyRings/ring/cr" +
				"yptoKeys/alice")}, []byte("ciphertext")); err == nil {
...
func TestGoogleCloudHSMRequiresProviderAttestation(t *testing.T) {
	...
	client := &http.Client{Transport: roundTripFunc(func(request *http.Request) (*http.Response, error) {
		if request.URL.Host == "oauth2.googleapis.com" {
			return &http.Response{
				StatusCode: http.StatusOK,
				Body:       io.NopCloser(strings.NewReader(`{"sub":"subject-a","expires_in":3600}`)),
				Header:     make(http.Header),
			}, nil
		}
		body := `{"plaintext":"` + base64.StdEncoding.EncodeToString(
			[]byte("wrapped-share"),
...
func TestGoogleTokenIntrospectionRejectsExpiredToken(t *testing.T) {
	client := &http.Client{Transport: roundTripFunc(func(request *http.Request) (*http.Response, error) {
		...
		return &http.Response{
			StatusCode: http.StatusOK,
			Body:       io.NopCloser(strings.NewReader(`{"sub":"subject-a","expires_in":"0"}`)),
			Header:     make(http.Header),
		}, nil
	})}
	unwrapper, err := NewGoogleCloudKMSUnwrapper([]byte("token-a"), client)
	...
	_, _, err = unwrapper.UnwrapMember(
		context.Background(),
		ExternalMemberContext{
			RepositoryID:   "repo-a",
			RootKeyVersion: 1,
