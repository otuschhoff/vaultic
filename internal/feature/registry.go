...
func init() {
	Flag.SetFlags(map[FlagName]FlagDesc{
		...
		DeprecateLegacyIndex:    {Type: Stable, Description: "disable support for index format used by vaultic 0.1.0. Use `vaultic repair index` to update the index if necessary."},
		DeprecateS3LegacyLayout: {Type: Stable, Description: "disable support for S3 legacy layout used up to vaultic 0.7.0. Use vaultic 0.17.3 to migrate if necessary."},
		...
		SlateDBAuthoritative:    {Type: Alpha, Description: "allow repositories with a validated SlateDB manifest to use the daemon-backed authoritative metadata engine"},
