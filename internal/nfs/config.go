...
func validateConfig(cfg Config) (validatedConfig, error) {
	...
	if !strings.HasPrefix(cfg.ExportName, "/") || cfg.ExportName == "/" || strings.Contains(cfg.ExportName[1:], "/") || strings.IndexByte(cfg.ExportName, 0) >= 0 {
	...
	if cfg.IdleTimeout < 0 || cfg.ConnectionIdleTimeout <= 0 || cfg.DrainTimeout <= 0 || cfg.MaxConnections <= 0 || cfg.MaxRequests <= 0 || cfg.MaxRequestSize < 40 || cfg.MaxRequestSize > int(^uint32(0)>>1) || cfg.HandleLimit <= 0 {
