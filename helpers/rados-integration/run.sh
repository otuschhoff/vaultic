...
go_builder=vaultic-rados-go-test
rust_builder=vaultic-rados-rust-test

cleanup() {
    docker rm -f "$cluster" >/dev/null 2>&1 || true
    docker network rm "$network" >/dev/null 2>&1 || true
    docker volume rm "$target" >/dev/null 2>&1 || true
    docker image rm "$go_builder" "$rust_builder" >/dev/null 2>&1 || true
}
...
common="--rm --platform linux/amd64 --network $network -e VAULTIC_RADOS_TEST_MONITORS=ceph-test:3300 -e VAULTIC_RADOS_TEST_KEY=$key -e VAULTIC_RADOS_TEST_DENIED_NAMESPACE=forbidden -v $root:/work"
...
docker run $common -v "$target:/work/vaulticdb/target" -w /work/vaulticdb --entrypoint /bin/sh "$rust_builder" -c \
    'cargo build --features rados && cargo test --features rados storage::rados::native::tests::live_rados_atomicity_reopen_and_isolation -- --nocapture'
docker run $common -v "$target:/work/vaulticdb/target" -w /work --entrypoint /bin/sh "$go_builder" -c \
    'go test -tags rados ./internal/repository -run TestLivePlacePackCopiesToRADOS -v'
...
docker run $common -e VAULTIC_RADOS_TEST_OSD_DOWN=1 -w /work --entrypoint /bin/sh "$go_builder" -c \
    'go test -tags rados ./internal/backend/rados -run TestNativeLiveOSDUnavailableIsBounded -v'
...
docker run $common -w /work --entrypoint /bin/sh "$go_builder" -c \
    'go test -tags rados ./internal/backend/rados -run TestNativeLiveRADOS -v'